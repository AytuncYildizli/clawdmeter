...
// Multi-line feed. With the chunked BLE write protocol lifting the per-write
// ceiling, the daemon ships up to MAX_ACTIVITY_EVENTS=8 events; we render the
// first VISIBLE_ROWS=6 on the card.
...
lv_obj_t* build(lv_obj_t* parent) {
    ...
    // Feed rows. y stride 28 fits 6 rows + title in 220px-2*pad card.
...
void refresh(const data::PayloadState& state) {
    ...
    if (state.activity_event_count == 0) {
        for (int i = 0; i < VISIBLE_ROWS; ++i) lv_label_set_text(g_rows[i], "");
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (ev.verb == '>') color = 0x44AA44;
        else if (ev.verb == '=') color = 0xFFCC00;
    ...
    for (int i = n; i < VISIBLE_ROWS; ++i) lv_label_set_text(g_rows[i], "");
}
